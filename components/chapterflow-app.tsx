...
export function ChapterFlowApp({ userEmail, userName, userRole }: ChapterFlowAppProps) {
  ...
  const [proposalSummary, setProposalSummary] = useState("Explores ethical responsibilities in edited collections that include practitioner and community voices.");
  ...
  const [comment, setComment] = useState("Thank you for the submission. Please strengthen the chapter structure and return the next version by the deadline.");
      ...
      {role === "public" ? (
        <section className="public-view">
          <div className="intro-band">
            ...
            <p>Contributors can browse open calls, choose the project they are joining, and begin with a short chapter proposal.</p>
          </div>
          ...
          <section className="panel registration-panel">
            ...
            <div className="form-grid">
              <label>Full name<input value={registeredName} onChange={(event) => setRegisteredName(event.target.value)} /></label>
              <label>Email<input value={registeredEmail} onChange={(event) => setRegisteredEmail(event.target.value)} /></label>
        ...
        <section className="author-view">
          ...
          <div className="author-grid">
            ...
            <section className="panel"><div className="section-heading"><p className="eyebrow">First action</p><h2>Submit chapter proposal</h2></div><p className="muted">Start with a title and short text proposal. Once approved, this panel becomes your first draft upload.</p><label>Proposed chapter title<input value={proposalTitle} onChange={(event) => setProposalTitle(event.target.value)} /></label><label>Proposal summary<textarea value={proposalSummary} onChange={(event) => setProposalSummary(event.target.value)} /></label><button className="primary" onClick={() => setProposalSubmitted(true)}>{proposalSubmitted ? "Proposal submitted" : "Submit proposal"}</button>{proposalSubmitted ? <div className="notice">Your proposal is now awaiting admin review.</div> : null}</section>
            <section className="panel"><div className="section-heading"><p className="eyebrow">Profile</p><h2>Biography and abstract</h2></div><label>Chapter abstract<textarea value={proposalSummary} onChange={(event) => setProposalSummary(event.target.value)} /></label><label>Author biography<textarea defaultValue={authorChapter.biography} /></label><button>Save profile details</button></section>
          </div>
        </section>
      )}
    </main>
  ...
  return (
    <section className="workspace">
      <aside className="panel sidebar">
        <div className="section-heading"><p className="eyebrow">Admin Control Centre</p><h2>LYIS Leadership With Mission Integrity</h2><p className="muted">Signed in as administrator. You can configure projects, manage roles, and oversee every submission.</p></div>
        ...
        <div className="admin-tools"><h3>Site controls</h3><label>Public call status<select defaultValue="Open"><option>Open</option><option>Closed</option><option>Draft</option></select></label><label>Chapter spaces<input defaultValue="12 chapters + 2 reserve spaces" /></label><button className="primary">Save public project settings</button></div>
      ...
      <aside className="panel detail-panel"><div className="section-heading"><p className="eyebrow">Review</p><h2>{selected.title}</h2></div><Meta selected={selected} /><div className="document-preview"><span>PDF / DOCX preview</span><p>{selected.abstract}</p></div><label>Decision<select value={decision} onChange={(event) => setDecision(event.target.value as Status)}>{statusOptions.map((status) => <option key={status}>{status}</option>)}</select></label><label>Admin comment<textarea value={comment} onChange={(event) => setComment(event.target.value)} /></label><div className="email-draft"><strong>{selectedTemplate.subject}</strong><p>{selectedTemplate.body}</p><p>{comment}</p></div><button className="primary">Approve email and update status</button></aside>
    </section>
  );
}
